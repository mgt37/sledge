...
var Comment    = require("../../app/models/blog/thinkAboutDepressionComment");
...
router.get("/", function(req, res){
    ...
    Comment.find({}, function(err, allComment){
        if(err){
            ...
            res.render("blog/posts/takeSomeTimeToThinkAboutDepression/index", {comment: allComment});
        }
    });
});
...
router.get("/:id", function(req, res){
    ...
    Comment.findById(req.params.id).populate("comments").exec(function(err, foundComment){
        if(err){
            ...
            res.render("blog/posts/takeSomeTimeToThinkAboutDepression/show", {comment: foundComment});    
        }
    });    
});
...
router.get("/:id/edit", blogMiddleware.checkThinkAboutDepressionCommentOwnership, function(req, res){
    Comment.findById(req.params.id, function(err, foundComment){
        if(err){
            ...
            res.render("blog/posts/takeSomeTimeToThinkAboutDepression/edit", {comment: foundComment});
        }
    });
});
