...
var Comment    = require("../../app/models/blog/lookingForAFlatComment");
...
router.get("/", function(req, res){
    ...
    Comment.find({}, function(err, allComment){
        if(err){
            ...
            res.render("blog/posts/whatIsImportantToConsiderWhenLookingForAFlat/index", {comment: allComment});
        }
    });
});
...
router.get("/:id", function(req, res){
    ...
    Comment.findById(req.params.id).populate("comments").exec(function(err, foundComment){
        if(err){
            ...
            res.render("blog/posts/whatIsImportantToConsiderWhenLookingForAFlat/show", {comment: foundComment});    
        }
    });    
});
...
router.get("/:id/edit", blogMiddleware.checkLookingForAFlatCommentOwnership, function(req, res){
    Comment.findById(req.params.id, function(err, foundComment){
        if(err){
            ...
            res.render("blog/posts/whatIsImportantToConsiderWhenLookingForAFlat/edit", {comment: foundComment});
        }
    });
});
