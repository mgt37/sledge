...
var Comment             = require("../../app/models/blog/balancePartTimeWorkComment");
...
router.get("/", function(req, res){
    ...
    Comment.find({}, function(err, allComment){
        if(err){
            ...
            res.render("blog/posts/getTheBalanceRightWithPartTimeWork/index", {comment: allComment});
        }
    });
});
...
router.get("/:id", function(req, res){
    ...
    Comment.findById(req.params.id).populate("comments").exec(function(err, foundComment){
        if(err){
            ...
            res.render("blog/posts/getTheBalanceRightWithPartTimeWork/show", {comment: foundComment});    
        }
    });    
});
...
router.get("/:id/edit", blogMiddleware.checkBalancePartTimeWorkCommentOwnership, function(req, res){
    Comment.findById(req.params.id, function(err, foundComment){
        if(err){
            ...
            res.render("blog/posts/getTheBalanceRightWithPartTimeWork/edit", {comment: foundComment});
        }
    });
});
