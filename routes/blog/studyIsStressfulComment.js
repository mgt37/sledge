...
var Comment    = require("../../app/models/blog/studyIsStressfulComment");
...
router.get("/", function(req, res){
    ...
    Comment.find({}, function(err, allComment){
        if(err){
            ...
            res.render("blog/posts/studyIsStressfulFindWaysToManageIt/index", {comment: allComment});
        }
    });
});
...
router.get("/:id", function(req, res){
    ...
    Comment.findById(req.params.id).populate("comments").exec(function(err, foundComment){
        if(err){
            ...
            res.render("blog/posts/studyIsStressfulFindWaysToManageIt/show", {comment: foundComment});    
        }
    });    
});
...
router.get("/:id/edit", blogMiddleware.checkStudyIsStressfulCommentOwnership, function(req, res){
    Comment.findById(req.params.id, function(err, foundComment){
        if(err){
            ...
            res.render("blog/posts/studyIsStressfulFindWaysToManageIt/edit", {comment: foundComment});
        }
    });
});
