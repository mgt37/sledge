...
var Comment    = require("../../app/models/blog/funOutsideOfStudyComment");
...
router.get("/", function(req, res){
    ...
    Comment.find({}, function(err, allComment){
        if(err){
            ...
            res.render("blog/posts/waysToHaveFunOutsideOfStudy/index", {comment: allComment});
        }
    });
});
...
router.get("/:id", function(req, res){
    ...
    Comment.findById(req.params.id).populate("comments").exec(function(err, foundComment){
        if(err){
            ...
            res.render("blog/posts/waysToHaveFunOutsideOfStudy/show", {comment: foundComment});    
        }
    });    
});
...
router.get("/:id/edit", blogMiddleware.checkFunOutsideOfStudyCommentOwnership, function(req, res){
    Comment.findById(req.params.id, function(err, foundComment){
        if(err){
            ...
            res.render("blog/posts/waysToHaveFunOutsideOfStudy/edit", {comment: foundComment});
        }
    });
});
