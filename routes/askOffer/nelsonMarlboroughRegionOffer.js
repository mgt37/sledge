...
router.get("/", function(req, res){
    ...
    if(req.query.search){
        ...
        NelsonMarlboroughRegionOffer.find({title: regex}, function(err, allNelsonMarlboroughRegionOffer){ //Change from name to other variable?
            if(err){
                console.log(err);
            } else {
                if(allNelsonMarlboroughRegionOffer.length <1){
                    noMatch = 'No titles match that query. Please try again.';
                }
                res.render("askOffer/nelsonMarlboroughRegion/offer/index", {nelsonMarlboroughRegionOffer: allNelsonMarlboroughRegionOffer, noMatch: noMatch});
            }
        });    
    } else {
        /*eval(require('locus'));*/
