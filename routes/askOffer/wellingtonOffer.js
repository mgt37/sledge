...
router.get("/", function(req, res){
    ...
    if(req.query.search){
        ...
        WellingtonOffer.find({title: regex}, function(err, allWellingtonOffer){ //Change from name to other variable?
            if(err){
                console.log(err);
            } else {
                if(allWellingtonOffer.length <1){
                    noMatch = 'No titles match that query. Please try again.';
                }
                res.render("askOffer/wellington/offer/index", {wellingtonOffer: allWellingtonOffer, noMatch: noMatch});
            }
        });    
    } else {
        /*eval(require('locus'));*/
