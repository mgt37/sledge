...
router.get("/", function(req, res){
    ...
    if(req.query.search){
        ...
        HamiltonOffer.find({title: regex}, function(err, allHamiltonOffer){ //Change from name to other variable?
            if(err){
                console.log(err);
            } else {
                if(allHamiltonOffer.length <1){
                    noMatch = 'No titles match that query. Please try again.';
                }
                res.render("askOffer/hamilton/offer/index", {hamiltonOffer: allHamiltonOffer, noMatch: noMatch});
            }
        });    
    } else {
        /*eval(require('locus'));*/
