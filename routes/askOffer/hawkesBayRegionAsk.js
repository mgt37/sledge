...
router.get("/", function(req, res){
    ...
    if(req.query.search){
        ...
        HawkesBayRegionAsk.find({title: regex}, function(err, allHawkesBayRegionAsk){ //Change from name to other variable?
            if(err){
                console.log(err);
            } else {
                if(allHawkesBayRegionAsk.length <1){
                    noMatch = 'No titles match that query. Please try again.';
                }
                res.render("askOffer/hawkesBayRegion/ask/index", {hawkesBayRegionAsk: allHawkesBayRegionAsk, noMatch: noMatch});
            }
        });    
    } else {
        /*eval(require('locus'));*/
