...
router.get("/", function(req, res){
    ...
    if(req.query.search){
        ...
        GisborneRegionOffer.find({title: regex}, function(err, allGisborneRegionOffer){ //Change from name to other variable?
            if(err){
                console.log(err);
            } else {
                if(allGisborneRegionOffer.length <1){
                    noMatch = 'No titles match that query. Please try again.';
                }
                res.render("askOffer/gisborneRegion/offer/index", {gisborneRegionOffer: allGisborneRegionOffer, noMatch: noMatch});
            }
        });    
    } else {
        /*eval(require('locus'));*/
...
router.post("/", middleware.isLoggedIn, function(req, res){
    ...
    var newGisborneRegionOffer = ({title: title, body: body, hourlyRate: hourlyRate, contactEmail: contactEmail, otherContact: otherContact, author: author});
