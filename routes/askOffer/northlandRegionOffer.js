...
router.get("/", function(req, res){
    ...
    if(req.query.search){
        ...
        NorthlandRegionOffer.find({title: regex}, function(err, allNorthlandRegionOffer){ //Change from name to other variable?
            if(err){
                console.log(err);
            } else {
                if(allNorthlandRegionOffer.length <1){
                    noMatch = 'No titles match that query. Please try again.';
                }
                res.render("askOffer/northlandRegion/offer/index", {northlandRegionOffer: allNorthlandRegionOffer, noMatch: noMatch});
            }
        });    
    } else {
        /*eval(require('locus'));*/
